function h = plot_gaussian_ellipsoid(m, C, sdwidth, npts, cor,axh)
%function h = plot_gaussian_ellipsoid(m, C, sdwidth, npts, axh,cor)
...
switch numel(m)
    case 2, h=show2d(m(:),C,sdwidth,npts,cor,axh);
...
%-----------------------------
function h = show2d(means, C, sdwidth, npts,cor, axh)
...
h = plot(bp(1,:), bp(2,:),cor, 'parent', axh);
%h = plot(bp(1,:), bp(2,:),color, 'parent', axh);
